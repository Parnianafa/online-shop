...
let basletLocal = localStorage.getItem("place");
if(basletLocal){
    cart = JSON.parse(basletLocal);
    initialCart();
}
...
let product3 = {
    id: 3,
    Image:"asset/img/Pancake Cupcakes.jpg",
    title:"Pancake Cupcakes",
    price:"5",
    model: "Cupcake",
    discount: "2"
}
let product4 = {
    id: 4,
    Image:"asset/img/The Best Creme Brulee Recipe.png",
    title:"Creme Brulee",
    price:"20",
    model: "Vanilla",
    discount: "10"
...
let product5 = {
    id: 5,
    Image:"asset/img/Berry Streusel Cheesecake.jpg",
    title:"Berry Streusel Cheesecake",
    price:"30",
    model: "Blueberry",
    discount: "10"
}
...
let product7 = {
    id: 7,
    Image:"asset/img/Blood Orange.jpg",
    title:"Orange Tart",
    price:"30",
    model: "Pastry cream",
    discount: "10"
...
let product8 = {
    id: 8,
    Image:"asset/img/Dalgona Coffee (Whipped Coffee).jpg",
...
let product9 = {
    id: 9,
    Image:"asset/img/Red Velvet.jpg",
    title:"Red Velvet with sweet cream cheese",
    price:"50",
    model: "chocolate",
    discount: "5"
}
let product10 = {
    id: 10,
    Image:"asset/img/Pink Lemonade Cocktail.jpg",
    title:"Pink Lemonade",
    price:"30",
    model: "Lemon",
    discount: "10"
...
$(document).ready(function(){
    ...
    for (let item of products) {
        var html = '<div class="product-box" data-id= '+ item.id +'>';
            ...
            html += '</div>';
            html += '<div class="product-explanation product-explanation-s">';
            ...
            html += '</div>';
            html += '<div class="product-bottom-box product-bottom-box-s">';
      ...
      if (searchValue && searchValue.length > 0) {
          ...
          if (foundedProduct && foundedProduct.length > 0) {
              ...
              for (const item of foundedProduct) {   //search into foundedProducts not all the products
                var html = '<div class="product-box product-box-s" data-id= '+ item.id +'>';
                html += '<div class="product-photo product-photo-s">';
                html += '<img src="'+ item.Image +'" class="product-photo-img" alt="">';
                html += '</div>';
                html += '<div class="product-explanation product-explanation-s">';
                html += '<span class="product-explanation-title product-title">Name: <span class="inner-value">'+ summaryString (item.title, 35) +'</span></span>';
                html += '<span class="product-explanation-title">Flavor: <span class="inner-value">'+ item.model +'</span></span>';
                html += '<del class="product-explanation-title">Old Price: <span class="inner-value">'+ item.price +' $</span></del>';
                html += '<span class="product-explanation-title">Price: <span class="inner-value">'+ (item.price - item.discount) +' $</span></span>';
                html += '</div>';
                html += '<div class="product-bottom-box product-bottom-box-s">';
                html += '<button class="product-bottom-more product-bottom-more-s" data-bs-toggle="modal" data-bs-target="#exampleModal">More To Features</button>';
                html += '<button class="product-bottom-cart product-bottom-cart-s">Add To Cart</button>';
                html += '</div>';
                html += '</div>';
                html += '</div>';
